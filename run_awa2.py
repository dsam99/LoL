import argparse
from process_data import get_data, create_dataloader, create_grad_dataloader
import numpy as np
import os
import sys
import random

from utils import str2bool, random_sample
from utils import auto_loss, grad_loss_awa2
from process_data import load_split_awa2_data
from models import MLP, define_model
...
def main():
	...
	for seed in seeds:
		...
		# loading split data
		(train_data, train_votes, train_grad, train_labels), (val_data, val_votes, val_grad, val_labels), \
			(test_data, _, _, test_labels) = load_split_awa2_data(args.dataset, seed)
		...
		if args.snorkel or args.mv:
			...
			if args.mv:
				...
				pseudolabs = label_model.predict_proba(fs_train[valid_inds])
		...
		# run our our LoL method
			...
			def auto_objective(trial):
				...
				optimizer = optim.Adam(model.parameters(), weight_decay=l2, lr=lr)
				batch_size = 128
				vote_loader = create_grad_dataloader(train_data, train_votes, train_grad, batch_size=batch_size, shuffle=True)
				...
				for i in range(num_epochs):
					...
					for x, vote in vote_loader:
						...
						if args.autoloss:
							loss = auto_loss(outputs, vote, device, coverage_weights, weights=weights)

						elif args.gradloss:
							loss = grad_loss_awa2(model, x, vote, wl_grad, device, coverage_weights,
			...
			if args.autoloss:
				print("Best Auto Loss Accuracy", test_accs[trial.number])
			if args.gradloss:
				print("Best Grad Loss Accuracy", test_accs[trial.number])
